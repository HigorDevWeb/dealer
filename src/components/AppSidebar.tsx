...
export function AppSidebar({ labels }: { labels: Labels }) {
  ...
  const handleLogout = async () => {
    // Exemplo: limpando o token salvo no localStorage
    if (typeof window !== "undefined") {
      localStorage.removeItem("strapi_jwt");
      sessionStorage.removeItem("strapi_jwt"); // caso use session
    }
    // Se você usa cookies, adicione lógica para remover o cookie aqui.

    // (Opcional) Chame a rota /auth/logout do Strapi se usar provider externo.
    // await fetch('http://localhost:1337/api/auth/logout', { method: 'POST' });
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <div className="flex items-center gap-3 px-2 py-3">
              <Link href="/" className="flex aspect-square size-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#ebff95] to-[#d4f564] shadow-lg overflow-hidden hover:ring-2 hover:ring-primary/50 transition-all duration-200">
                <img
              ...
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-bold text-lg">{labels.siteTitle}</span>
      ...
      <SidebarContent className="px-2">
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            ...
            >
              <span className="font-semibold text-sm">{labels.toolsLabel}</span>
              {openTools ? (
            ...
            <div
              id="tools-menu"
              className={`overflow-hidden transition-all duration-200 ${openTools ? "max-h-52 opacity-100 mt-1" : "max-h-0 opacity-0"
                }`}
            >
            ...
            <SidebarMenu className="space-y-1">
              <SidebarMenuItem>
                <SidebarMenuButton className="p-4 hover:bg-accent/50 transition-all duration-200 h-auto">
                  ...
                  <span className="text-sm font-medium">{labels.preferencesLabel}</span>
                </SidebarMenuButton>
              </SidebarMenuItem>
              <SidebarMenuItem>
                <div className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium">{labels.themeLabel}</span>
                  </div>
                  <ModeToggle />
                </div>
              </SidebarMenuItem>
